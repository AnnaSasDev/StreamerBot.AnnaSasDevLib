...
public class ObsUtilities(IStreamerBotUtilities utilities) : IObsUtilities {
    ...
    private Lazy<ObsData?> LazyObsData { get; } = new(static () => AssetFileProvider.TryParseJsonAssetFile(DataFileName, out ObsData? data) 
        ? data 
        : null
    );
    ...
    // Methods
    // -----------------------------------------------------------------------------------------------------------------
    public bool TryGetSourceReference(string referenceId, [NotNullWhen(true)] out string? sceneName, [NotNullWhen(true)] out string? sourceName) {
        sourceName = null;
        sceneName = null;
        if (LazyObsData.Value is not {} data) return false;
        if (!data.SourceReferences.TryGetValue(referenceId, out ObsSourceReference? sourceReference)) return false;
